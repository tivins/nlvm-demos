...
methods `filter`/`sort`/`forEach`/`map`, and an exhaustive `match`
expression — none of which are used by the earlier demos.
...
- `src/Main.nl` — declares a closure `isPassing` (an anonymous function bound
  ...
  and `forEach()`; curves every score with `map()`; then converts each
  curved score to a letter grade with a `match(int)` expression (which
  requires a `default` arm, since `int`'s value domain is unbounded).
...
> captured/outer state, which works reliably. A few other documented
> features (`enum`, explicit function-type variable declarations, the `??`
> operator) also aren't implemented by the current compiler yet, so this
> demo avoids them.
